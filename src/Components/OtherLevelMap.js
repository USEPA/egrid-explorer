...
class OtherLevelMap extends Component {
    render() {
      let marginTop = 80;
        ...
        let layer = this.props.layer, label_width = this.props.layer_type==='grid gross loss rates' ? 65 : (this.props.layer_type==='state' ? 20 : 40), label_height = this.props.layer_type!=='grid gross loss rates' ? 14 : 22;
        ...
        let title = <text transform={'translate(' + this.props.width/2 + ',' + marginTop/2 + ')'} style={{fontSize: '1.2em', fontWeight: 'bold', fill: '#000', className: 'title', textAnchor: 'middle'}}>{this.props.title}</text>;
        ...
        let labels = layer.features.map((d,i)=><g key={'path' + i + '_label'}>
                                                <rect x={d.properties.centroid[0]-label_width/2} y={d.properties.centroid[1]}
                                                width={label_width} height={label_height}
                                                rx={4}
                                                style={{fill: '#fff', stroke: this.props.layer_type!=='grid gross loss rates'? 'none' : '#000'}}/>
                                                <text
                                                x={d.properties.centroid[0]} y={d.properties.centroid[1]+label_height*0.8}
